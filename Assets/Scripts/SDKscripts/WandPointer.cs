        ...
        protected virtual void CheckCanvas(GraphicRaycaster pGraphicsRaycaster, Vector3 pPosition)
        {
            ...
            if (results.Count > 0)
            {
                for (int i = 0; i < results.Count; i++)
                {
                    ...
                    Transform parentTransform = results[i].gameObject.transform.parent;
                    Selectable selectable = parentTransform.gameObject.GetComponent<Selectable>();
             
                   
                    Debug.Log("Found selectable " + selectable.ToString());
                    if (selectable != null)
                    {
                        foundSelectable = true;
                        Debug.Log("Seelctable found = true");
